...
public class TradeSettingsController {
    ...
    @Autowired
    public TradeSettingsController(TradeSettingsService tradeSettingsService, FinancialInstrumentService financialInstrumentService) {
...
//        var isMax = tradeSettings.getMaximumDefinition() - 0.0 > epsilon;
    ...
    public String showUpdateForm(@PathVariable("id") long id, Model model) {
        ...
        if (tradeSettings == null) {
            tradeSettings = this.tradeSettingsService.findById(id)
...
//            redirectAttributes.addFlashAttribute("tradingStrategyTypes", TradingStrategy.values());
        ...
        this.tradeSettingsService.save(tradeSettings);
    ...
    public String update(@PathVariable("id") long id, @Valid TradeSettings tradeSettings, BindingResult result, Model model, HttpServletRequest request) {
        ...
        this.tradeSettingsService.update(tradeSettings);
        return "redirect:/trade_settings/index";
    }

    @GetMapping("trade_settings/delete/{id}")
    public String delete(@PathVariable("id") long id, Model model) {
        TradeSettings tradeSettings = this.tradeSettingsService.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Invalid user Id:" + id));
        this.tradeSettingsService.delete(tradeSettings);
    ...
    public String showUserList(Model model) {
        model.addAttribute("tradesSettings", tradeSettingsService.findAll());
